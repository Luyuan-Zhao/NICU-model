...
#---------------------------------------------------------------------------------------------------------------------
# 12. OAT sensitivity analysis  
using CSV, DataFrames, Statistics

# function to perform one-at-a-time (OAT) sensitivity analysis
function OAT_sensitivity(param_name::Symbol, values::Vector; N_runs=100)
    results = DataFrame(
        run=Int[],
        param=Float64[],
        peak_infection=Float64[],
    ...
    for val in values
        println("Running $param_name = $val ...")
        new_params = merge(DEFAULT_PARAMS, NamedTuple{(param_name,)}((val,)))
        df = collect_outputs_table(N_runs, new_params)
...
# Parameterization for sensitivity analysis
param_ranges = Dict(
    :p_empiric_abx => [0.05, 0.1, 0.15, 0.2, 0.3],
    :abx_course => [3, 5, 7, 10],
    :p_decontam_hcw => [0.2, 0.4, 0.6, 0.8, 0.9],
    :p_decontam_dev => [0.1, 0.3, 0.5, 0.7],
    :p_clean_env => [0.1, 0.3, 0.5, 0.7, 0.9]
)

# Main loop to run OAT sensitivity analysis for each parameter
for (param, vals) in param_ranges
    df = OAT_sensitivity(param, vals, N_runs=100)
    filename = "sensitivity_$(param).csv"
