# ESBL-producing E. coli Dynamic ABM in Tanzanian NICUs (version 1.4)
#Luyuan Zhao 
#2025-05-22 
...
# Environment - Stores all agents and parameters of the simulation.(Mutable)
...
function main()
    ...
    plt = plot(
        title="ESBL-producing E. coli Dynamics in NICU (with Total Recoveries)",
        xlabel="Days",
...
function run_multiple!(N_runs::Int=100, days::Int=90)                                      # Define a function to run multiple simulations : times & days
    ...
    abx_per_baby_mean = round(mean(abx_days_per_baby_list), digits=2)
    abx_per_baby_sd = round(std(abx_days_per_baby_list), digits=2)
    println("Avg ABX days per baby (mean ± SD): $abx_per_baby_mean ± $abx_per_baby_sd")
end
