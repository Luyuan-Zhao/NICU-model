# ESBL-producing Enterobacterales Dynamic ABM in Tanzanian NICUs (version 1.13)
...
const DEFAULT_PARAMS = (
    ...
    p_progress=0.11, #0.03,
    p_recovery_S=0.20,
    p_recovery_R=0.12,
    abx_course=5,
    p_selection=0.05,

    p_decontam_hcw=0.56, #0.4,
...
function run_multiple!(N_runs::Int=100, params=DEFAULT_PARAMS)
...
function collect_outputs_table(N_runs::Int=100, params=DEFAULT_PARAMS)
...
df = collect_outputs_table(100)


#---------------------------------------------------------------------------------------------------------------------

# 12. Define scenarios and run simulations
# Baseline settings
BASELINES = Dict(
    "Low" => merge(DEFAULT_PARAMS, (
        β_hb=DEFAULT_PARAMS.β_hb * 0.85,
        β_db=DEFAULT_PARAMS.β_db * 0.85,
        p_env_to_baby=DEFAULT_PARAMS.p_env_to_baby * 0.85
    )),
    "Mid" => DEFAULT_PARAMS,
    "High" => merge(DEFAULT_PARAMS, (
        β_hb=DEFAULT_PARAMS.β_hb * 1.15,
        β_db=DEFAULT_PARAMS.β_db * 1.15,
        p_env_to_baby=DEFAULT_PARAMS.p_env_to_baby * 1.15
    ))
)
...
STRATEGIES = Dict(
    ...
    "A" => (p_empiric_abx=0.1,),
    "B" => (abx_course=3,),
    "C" => (p_decontam_hcw=0.9,),
    "D" => (p_decontam_dev=0.8,),
    "E" => (p_clean_env=0.8,),
    "F" => (p_empiric_abx=0.1, abx_course=3),
    "G" => (p_decontam_hcw=0.9, p_decontam_dev=0.8, p_clean_env=0.8),
    "H" => (p_empiric_abx=0.1, abx_course=3, p_decontam_hcw=0.9, p_decontam_dev=0.8, p_clean_env=0.8)
)

# Store all results in a DataFrame
all_results = DataFrame()

# Iterate through all combinations of baselines and strategies
for (bname, bparams) in BASELINES
    for (sname, sparams) in STRATEGIES
        scen_name = "$(bname)_$(sname)"
        combined_params = merge(bparams, sparams)

        println("Running: ", scen_name)
        df = collect_outputs_table(100, combined_params)

        # Add scenario, baseline, and strategy names to the DataFrame
        df.scenario .= scen_name
        df.baseline .= bname
        df.strategy .= sname
        df.prevalence_I_R .= df.cumulative_I_R ./ (combined_params.N_babies * combined_params.days_total)
...
CSV.write("all_scenarios_full.csv", all_results)
println("All 24 scenarios finished and saved to all_scenarios_full.csv")
