# ESBL-producing Enterobacterales Dynamic ABM in Tanzanian NICUs (version 1.12)
...
function daily_step!(env::Environment, rng::AbstractRNG)
    ...
    env_is_contaminated = rand(rng) > env.p_clean_env
...
const SCENARIO_SETTINGS = Dict(
    "Baseline" => DEFAULT_PARAMS,
    "A" => merge(DEFAULT_PARAMS, (p_empiric_abx=0.1,)),
    "B" => merge(DEFAULT_PARAMS, (abx_course=3,)),
    "C" => merge(DEFAULT_PARAMS, (p_decontam_hcw=0.9,)),
    "D" => merge(DEFAULT_PARAMS, (p_decontam_dev=0.8,)),
    "E" => merge(DEFAULT_PARAMS, (p_clean_env=0.8,)),
    "F" => merge(DEFAULT_PARAMS, (p_empiric_abx=0.1, abx_course=3)),
    "G" => merge(DEFAULT_PARAMS, (p_decontam_hcw=0.9, p_decontam_dev=0.8, p_clean_env=0.8)),
    "H" => merge(DEFAULT_PARAMS, (p_empiric_abx=0.1, abx_course=3, p_decontam_hcw=0.9, p_decontam_dev=0.8, p_clean_env=0.8))
)
 # Define a function to run all scenarios and collect outputs
 function run_all_scenarios_combined(N_runs::Int=100)
    combined_df = DataFrame()

    for (scenario, params) in SCENARIO_SETTINGS
        println("Running scenario: $scenario")
        df = collect_outputs_table(N_runs, params)
        df.scenario .= scenario  # 添加新列：scenario
        append!(combined_df, df) # 合并到总表格中
    end

    println("Saving all scenario results into one CSV...")
    CSV.write("all_scenarios_combined.csv", combined_df)
    return combined_df
end

all_df = run_all_scenarios_combined(100)
